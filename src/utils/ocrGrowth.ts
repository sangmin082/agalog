...
export async function extractGrowthFromImage(
  ...
  const prompt =
    '이 이미지는 아기 성장 기록지 또는 병원 기록입니다.\n' +
    '이미지에서 다음 값들을 찾아서 JSON으로만 응답해주세요 (설명 없이 JSON만):\n' +
    '{"weightKg": <몸무게 kg 숫자, 없으면 null>, "heightCm": <키 cm 숫자, 없으면 null>, ' +
    '"headCm": <머리둘레 cm 숫자, 없으면 null>, "date": <측정날짜 YYYY-MM-DD, 없으면 null>}\n' +
